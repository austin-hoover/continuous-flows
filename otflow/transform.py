...
class AddUniformNoise(object):
    ...
    def __call__(self, samples):
        ...
        samples = logit(samples / 256.0, self.alpha)
        return samples
...
class ZeroPadding(object):
    ...
    def __call__(self, samples):
        ...
        tmp = np.zeros((32, 32))
        tmp[
            self.num : samples.shape[0] + self.num,
            self.num : samples.shape[1] + self.num,
        ] = samples
        return tmp
