...
parser.add_argument(
    "--nt_val", type=int, default=16, help="number of time steps for validation"
)
...
parser.add_argument(
    "--drop_freq", type=int, default=5000, help="how often to decrease learning rate"
)
parser.add_argument(
    "--lr_drop",
    type=float,
    default=10.0 ** (0.5),
    help="how much to decrease learning rate (divide by)",
)
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, _ = getLoader(
        args.data,
        args.batch_size,
        args.val_batch_size,
        augment=False,
    ...
    # -----------AutoEncoder ------------------------------------------------------------
    if (
        args.autoenc is None
    ):  # if no trained encoder-decoder is provided, then train one
        ...
        autoEnc.load_state_dict(
            checkpt["state_dict"], strict=False
        )  # doesnt load the buffers
    ...
    log_msg = "{:5s}  {:6s}  {:7s}   {:9s}  {:9s}  {:9s}  {:9s}     {:9s}  {:9s}  {:9s}  {:9s} ".format(
        "iter",
        " time",
        "lr",
        "loss",
        ...
        for data in train_loader:
            ...
            if itr % args.viz_freq == 0:
                with torch.no_grad():
                    ...
                    p_samples = (p_samples - autoEnc.mu) / (
                        autoEnc.std + args.eps
                    )  # normalize
                    ...
                    plot4(
                        ...
                        nt_val,
                        sPath,
                        sTitle="loss {:.2f}  ,  C {:.2f}".format(
                            best_loss, best_costs[1]
                        ),
                    )
                    ...
                    sPath = os.path.join(
                        ...
                        "figs",
                        start_time
                        + "_class{:d}_imshow{:04d}.png".format(args.conditional, itr),
                    )
                    genModel = integrate(
                        y[:, 0:d], net, [1.0, 0.0], nt_val, stepper="rk4", alph=net.alph
                    )
                    genModel = genModel[:, 0:d]
                    genDecoded = autoEnc.decode(
                        genModel * (autoEnc.std + args.eps) + autoEnc.mu
                    )  # de-normalize and decode
                    pDecoded = autoEnc.decode(
                        p_samples * (autoEnc.std + args.eps) + autoEnc.mu
                    )  # de-normalize and decode
