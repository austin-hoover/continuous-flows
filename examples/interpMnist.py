...
if __name__ == "__main__":
    ...
    _, _, test_loader = getLoader(
        args.data, args.batch_size, args.batch_size, augment=False, hasGPU=cf.gpu
    )
    ...
    autoEnc.load_state_dict(
        checkpt["autoencoder"], strict=False
    ...
    with torch.no_grad():
        ...
        z1 = integrate(x0[:, 0:d], net, [0.0, 1.0], nt, stepper="rk4", alph=net.alph)[
            0:d
        ]  # flow to rho_1
        z1 = z1[:, 0:d]
        ...
        for col in range(nInterp + 1):
            ...
            for row in range(1, nInterp):
                recastZ[row * (nInterp + 1) + col, :] = top + 1.0 / nInterp * row * (
                    bot - top
                )

        gen = integrate(
            recastZ[:, 0:d], net, [1.0, 0.0], nt, stepper="rk4", alph=net.alph
        )[:, 0:d]
        ...
        for i in range(nInterp + 1):
            for j in range(nInterp + 1):
                ...
                if (
                    (i == 0 and j == 0)
                    or (i == nInterp and j == 0)
                    or (i == 0 and j == nInterp)
                    or (i == nInterp and j == nInterp)
                ):
                    # Create a Rectangle patch
                    rect = patches.Rectangle(
                        (0, 0), 27, 27, linewidth=2, edgecolor="r", facecolor="none"
                    )
