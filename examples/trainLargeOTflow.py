...
parser.add_argument(
    "--data",
    choices=["power", "gas", "hepmass", "miniboone", "bsds300", "mnist"],
    type=str,
    default="miniboone",
)
...
parser.add_argument(
    "--nt_val", type=int, default=10, help="number of time steps for validation"
)
...
parser.add_argument(
    "--val_freq", type=int, default=def_val_freq
)  # validation frequency needs to be less than viz_freq or equal to viz_freq
        ...
        for x0 in batch_iter(data.trn.x, shuffle=True):
            ...
            if itr % args.val_freq == 0 or itr == args.niters:
                ...
                with torch.no_grad():
                    ...
                    net.train()
                    log_message += " no improve: {:d}/{:d}".format(
                        n_vals_wo_improve, args.early_stopping
                    )
            ...
            if itr % args.viz_freq == 0:
                with torch.no_grad():
                    ...
                    )  # sampling from rho_1 / standard normal
                    ...
                    plot4(
                        ...
                        nt_val,
                        sPath,
                        sTitle="loss {:.2f}  ,  C {:.2f}".format(best_loss, best_cs[1]),
                    )
            ...
            if (
                args.drop_freq == 0
            ):  # if set to the code setting 0 , the lr drops based on validation
