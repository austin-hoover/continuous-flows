...
def_resume = (
    "experiments/cnf/toy/pretrained/pretrained_swissroll_alph30_15_m32_checkpt.pth"
)
...
if __name__ == "__main__":
    ...
    p_samples = cvt(
        torch.Tensor(toy_data.inf_train_gen(args.data, batch_size=nSamples))
    )
    ...
    with torch.no_grad():
        ...
        modelFx = integrate(
            p_samples[:, 0:d], net, [0.0, 1.0], args.nt, stepper="rk4", alph=net.alph
        )
        modelFinvfx = integrate(
            modelFx[:, 0:d], net, [1.0, 0.0], args.nt, stepper="rk4", alph=net.alph
        )
        modelGen = integrate(
            y[:, 0:d], net, [1.0, 0.0], args.nt, stepper="rk4", alph=net.alph
        )

        print(
            "          {:9s}  {:9s}  {:11s}  {:9s}".format(
                "loss", "L (L_2)", "C (loss)", "R (HJB)"
            )
        )
        print(
            "[TEST]:   {:9.3e}  {:9.3e}  {:11.5e}  {:9.3e}".format(
                test_loss, test_cs[0], test_cs[1], test_cs[2]
            )
        )
        ...
        invErr = np.mean(
            np.linalg.norm(
                ...
                - modelFinvfx[:, :d].detach().cpu().numpy(),
                ord=2,
                axis=1,
            )
        )
        ...
        def normpdf(x):
            ...
            cov = torch.ones(
                1, d, device=x.device, dtype=x.dtype
        ...
        h3, _, _, map3 = ax.hist2d(
            genModel.detach().cpu().numpy()[:, d1],
            genModel.detach().cpu().numpy()[:, d2],
            range=extent,
            bins=nBins,
        )
        h3 = h3 / (nSamples)
