...
if not torch.cuda.is_available():
    print(
        "No gpu found. If you wish to run on a CPU, remove the cuda specific lines, then run again."
    )
    exit(1)
...
def compareTrace(domain, d, seed=0):
    ...
    for iDomain, num_hutchinsons in enumerate(domain):
        ...
        start.record()
        rad = (1 / math.sqrt(num_hutchinsons)) * (
            (torch.rand(n_samples, d + 1, num_hutchinsons, device=device) < 0.5).float()
            * 2
            - 1
        )
        ...
        trace_error = torch.norm(exact_trace - trace_acc) / torch.norm(
            exact_trace
        )  # compute error
        print(
            "{:4d} hutchinson vectors.  time= {:9.6f} , rel. error = {:9.7f}".format(
                num_hutchinsons, ad_time, trace_error
            )
        )
...
if __name__ == "__main__":
    ...
    plotTraceCompare(
        ...
        traceErrorMiniArray,
        traceErrorBSDSArray,
        traceErrorMNISTArray,
        lTimeExact,
        "image/traceComparison/",
    )
