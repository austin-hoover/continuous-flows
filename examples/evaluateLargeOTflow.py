...
parser.add_argument(
    "--resume",
    type=str,
    default="experiments/cnf/large/pretrained/pretrained_miniboone_checkpt.pth",
)

parser.add_argument(
    "--nt", type=int, default=18, help="number of integration time steps"
)
...
if __name__ == "__main__":
    ...
    log_msg = "{:4s}        {:9s}  {:9s}  {:11s}  {:9s}".format(
    ...
    with torch.no_grad():
        ...
        logger.info(
            "Testing Time:          {:.2f} seconds with {:} parameters".format(
                time.time() - end, count_parameters(net)
            )
        )
        if args.long_version:
            logger.info(
                "FFJORD's Testing Time: {:.2f} seconds with {:} parameters".format(
                    ffjordTime, ffjordWeights
                )
            )
        ...
        for i, x0 in enumerate(batch_iter(testData, batch_size=args.batch_size)):
            x0 = cvt(x0)
            fx = integrate(
                x0[:, 0:d], net, [0.0, 1.0], nt_test, stepper="rk4", alph=net.alph
            )
            finvfx = integrate(
                fx[:, 0:d], net, [1.0, 0.0], nt_test, stepper="rk4", alph=net.alph
            )

            # consolidate fx and finvfx into one spot
            batchSz = x0.shape[0]
            modelFx[idx : idx + batchSz, 0:d] = fx[:, 0:d].detach().cpu().numpy()
            modelFinvfx[idx : idx + batchSz, 0:d] = (
                finvfx[:, 0:d].detach().cpu().numpy()
            )
            idx = idx + batchSz

        # logger.info("model inv error:  {:.3e}".format(np.linalg.norm(testData.numpy() - modelFinvfx) / nex)) # initial bug
        logger.info(
            "model inv error:  {:.3e}".format(
                np.mean(np.linalg.norm(testData.numpy() - modelFinvfx, ord=2, axis=1))
            )
        )
        if args.long_version:
            logger.info(
                "FFJORD inv error: {:.3e}".format(np.array(hf["invErr"]).item())
            )
        ...
        for i, y in enumerate(batch_iter(normSamples, batch_size=args.batch_size)):
            y = cvt(y)
            finvy = integrate(
                y[:, 0:d], net, [1.0, 0.0], nt_test, stepper="rk4", alph=net.alph
            )
        ...
        if (
            not args.long_version
        ):  # when running abbreviated style, use smaller sample sizes to compute mmd so its quicker
            nSamples = min(
                testData.shape[0], modelGen.shape[0], 3000
            )  # number of samples for the MMD
        ...
        print(
            "MMD( ourGen   , rho_0 ),  num(ourGen)={:d}    , num(rho_0)={:d} : {:.5e}".format(
                modelSamps.shape[0], testSamps.shape[0], mmd(modelSamps, testSamps)
            )
        )
        if args.long_version:
            ffjordSamps = ffjordGen
            print(
                "MMD( FFJORDGen, rho_0 ),  num(FFJORDGen)={:d} , num(rho_0)={:d} : {:.5e}".format(
                    ffjordSamps.shape[0],
                    testSamps.shape[0],
                    mmd(ffjordSamps, testSamps),
                )
            )
        ...
        for d1 in range(
            0, d - 1, 2
        ):  # plot 2-D slices of the multivariate distribution
            d2 = d1 + 1
            ...
            im1, _, _, map1 = axs[0, 0].hist2d(
                testData[:, d1], testData[:, d2], range=boundsRho0, bins=nBins
            )
            axs[0, 0].set_title(r"$x \sim \rho_0(x)$")

            im2, _, _, map2 = axs[0, 1].hist2d(
                modelFx[:, d1], modelFx[:, d2], range=bounds, bins=nBins
            )
            axs[0, 1].set_title(r"$f(x)$")

            im3, _, _, map3 = axs[1, 0].hist2d(
                normSamples[:, d1], normSamples[:, d2], range=bounds, bins=nBins
            )
            axs[1, 0].set_title(r"$y \sim \rho_1(y)$")

            im4, _, _, map4 = axs[1, 1].hist2d(
                modelGen[:, d1], modelGen[:, d2], range=boundsRho0, bins=nBins
            )
            axs[1, 1].set_title(r"$f^{-1}(y)$")

            if args.long_version:
                im5, _, _, map5 = axs[0, 2].hist2d(
                    ffjordFx[:, d1], ffjordFx[:, d2], range=bounds, bins=nBins
                )
                axs[0, 2].set_title(r"FFJORD $f(x)$")

                im6, _, _, map6 = axs[1, 2].hist2d(
                    ffjordGen[:, d1], ffjordGen[:, d2], range=boundsRho0, bins=nBins
                )
                axs[1, 2].set_title(r"FFJORD $f^{-1}(y)$")
            else:
                placeholder = 100.0 * np.ones_like(testData[:, 0])
                im5, _, _, map5 = axs[0, 2].hist2d(
                    placeholder, placeholder, range=bounds, bins=nBins
                )
                axs[0, 2].set_title("placeholder")

                im6, _, _, map6 = axs[1, 2].hist2d(
                    placeholder, placeholder, range=boundsRho0, bins=nBins
                )
